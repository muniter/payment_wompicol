odoo.define("payment_wompicol.payment_form", ['web.public.widget', 'payment.payment_form'], function (require) {
    'use strict';

    var PaymentForm = require('payment.payment_form');
    var publicWidget = require('web.public.widget');
    console.log(publicWidget, PaymentForm);
    PaymentForm.prototype.payEvent = function (ev) {
        ev.preventDefault();
        var form = this.el;
        ...
        if (checked_radio.length === 1 && this.isNewPaymentRadio(checked_radio) && checked_radio.data('provider') === 'wompicol') {
        ...
        if (checked_radio.length === 1) {
            ...
            // if the user is going to pay with a form payment, then
                ...
                if ($tx_url.length === 1) {
                    ...
                    }).then(function (result, request_type) {
                        if (result) {
                            // if the server sent us the html form, we create a form element
                            var newForm = document.createElement('form');
                            newForm.setAttribute("method", request_type);
